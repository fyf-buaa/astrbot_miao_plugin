...
async def calc_damage_handler(e: Any) -> Any:
    ...
    if not char_name:
        return await e.reply("请指定角色，如 #雷神伤害 或 #星铁遐蝶伤害")
    ...
    if not result:
        return await e.reply("伤害计算失败（角色面板数据可能不完整）")
